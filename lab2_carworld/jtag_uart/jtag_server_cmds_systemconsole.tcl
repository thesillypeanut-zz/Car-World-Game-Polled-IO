...
proc jtag_read {addr} {
    ...
    global jtag_avalon_bridge_service_path
    
    set result [master_read_32 $jtag_avalon_bridge_service_path $addr 0x1]
    set result [format "0x%.8X" $result]
...
proc jtag_write {addr data} {
    ...
    master_write_32 $jtag_avalon_bridge_service_path $addr $data
...
proc client_handler {client} {
	if {[eof $client]} {
		...
		if {[catch {eval $cmd} rsp]} {
			puts "SERVER ($client): Invalid command from the client: $rsp"
		} else {
